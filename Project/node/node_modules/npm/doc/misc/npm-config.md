...
When publishing scoped packages, the access level_1 defaults to `restricted`.  If
you want your scoped package to be publicly viewable (and installable) set
`--access=public`. The only valid values for `access` are `public` and
`restricted`. Unscoped packages _always_ have an access level_1 of `public`.
...
What level_1 of logs to report.  On failure, *all* logs are written to
...
1. devDependencies are not installed at the topmost level_1 when running
